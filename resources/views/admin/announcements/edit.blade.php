...
<div class="admin-card" style="max-width: 800px;">
    <div class="admin-card-header">
        <div>
            <h3 class="admin-card-title">Edit Announcement</h3>
            <p class="admin-card-description">Update the announcement details</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.announcements.update', $announcement) }}" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="message" name="message" rows="3"
                    class="admin-form-input resize-none" placeholder="Enter your announcement message">{{ old('message', $announcement->message) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="code" name="code" type="text" value="{{ old('code', $announcement->code) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="animation" name="animation" class="admin-form-input">
                        <option value="slide" {{ old('animation', $announcement->animation) === 'slide' ? 'selected' : '' }}>Slide Up</option>
                        <option value="typewriter" {{ old('animation', $announcement->animation) === 'typewriter' ? 'selected' : '' }}>Typewriter</option>
                    </select>
                </div>

                <div>
                    <label for="duration" class="admin-form-label">Duration (ms) <span class="text-destructive">*</span></label>
                    <input id="duration" name="duration" type="number" value="{{ old('duration', $announcement->duration) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input id="link" name="link" type="url" value="{{ old('link', $announcement->link) }}"
                        class="admin-form-input" placeholder="https://example.com">
                </div>

                <div>
                    <label for="link_text" class="admin-form-label">Link Text (Optional)</label>
                    <input id="link_text" name="link_text" type="text" value="{{ old('link_text', $announcement->link_text) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input id="sort_order" name="sort_order" type="number" value="{{ old('sort_order', $announcement->sort_order) }}"
            ...
            <div class="flex items-center justify-end gap-4 pt-4 border-t border-gray-200">
                ...
                <button type="submit" class="admin-btn admin-btn-primary">Update Announcement</button>
            </div>
        </form>
    </div>
</div>
