...
<div class="admin-card">
    <div class="admin-card-header">
        <div>
            <h3 class="admin-card-title">Edit Product</h3>
            <p class="admin-card-description">Update product information</p>
        </div>
    ...
    <div class="admin-card-body">
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 space-y-4">
                    <div class="admin-form-group">
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="admin-form-input">
                    </div>
                    <div class="admin-form-group">
                        <label class="admin-form-label">Slug</label>
                        <input type="text" name="slug" value="{{ old('slug', $product->slug) }}" required class="admin-form-input">
                    </div>
                    <div class="admin-form-group">
                ...
                <div class="lg:col-span-1">
                    <div class="admin-card p-4 sticky" style="top: 24px;">
                        ...
                        <p class="text-sm text-muted-foreground mb-4">Upload product images via drag & drop. Click an image to set it as primary. Click the X to delete.</p>
                        ...
                        <div id="image-preview-container" class="grid grid-cols-3 gap-3 mt-4">
                            @foreach($product->images as $image)
                                @include('admin.products._image_preview', [
                                    'image' => $image,
            ...
            <button type="submit" class="admin-btn admin-btn-primary mt-6">Update Product</button>
        </form>
    </div>
</div>

@php
    $existingVariants = $product->variants->mapWithKeys(fn ($v) => ["{$v->size_id}_{$v->color_id}" => [
        'sku' => $v->sku,
        'stock' => $v->stock,
    ...
    const pollInit = setInterval(() => {
        if (typeof window.AdminVariants !== 'undefined' && typeof window.AdminImageGallery !== 'undefined') {
            ...
            window.AdminImageGallery.init({
                ...
                hasProduct: true,
                productId: {{ $product->id }},
                indexUrl: '{{ route('admin.products.images.index', $product) }}',
                uploadUrl: '{{ route('admin.products.images.upload', $product) }}',
                primaryUrl: (imageId) => '{{ route('admin.products.images.primary', [$product, '__IMAGE_ID__']) }}'.replace('__IMAGE_ID__', imageId),
                deleteUrl: (imageId) => '{{ route('admin.products.images.delete', [$product, '__IMAGE_ID__']) }}'.replace('__IMAGE_ID__', imageId),
            });
        }
    }, 10);
})();
